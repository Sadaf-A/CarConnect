...
const ProductInfo = () => {
  ...
  useEffect(() => {
    const fetchProductInfo = async () => {
      try {
        ...
        const response = await axios.get(`https://refined-spirit-456112-e0.as.r.appspot.com/api/cars/get-car/${carId}`, {
  ...
  const handleUpdateCar = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://refined-spirit-456112-e0.as.r.appspot.com/api/cars/update-car/${carId}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteCar = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://refined-spirit-456112-e0.as.r.appspot.com/api/cars/delete-car/${carId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
