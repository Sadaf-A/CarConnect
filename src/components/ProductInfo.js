...
const ProductInfo = () => {
  ...
  useEffect(() => {
    const fetchProductInfo = async () => {
      try {
        ...
        const response = await axios.get(`https://test-web-site-464607.df.r.appspot.com/api/cars/get-car/${carId}`, {
  ...
  const handleUpdateCar = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`https://test-web-site-464607.df.r.appspot.com/api/cars/update-car/${carId}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteCar = async () => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://test-web-site-464607.df.r.appspot.com/api/cars/delete-car/${carId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
